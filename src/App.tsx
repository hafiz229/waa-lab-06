...
const App = () => {
  ...
  const handleDeleteOnClick = (id: Number) => {
    const updatedUserList = userList?.filter(({rpid}) => rpid !== id);
    ...
    <div className="app">
      ...
      <div className="reply-navigation">
        <ul className="nav-bar">
          <li className="nav-title">
            ...
            {/* Like */}
            <span className="total-reply">{userList?.length}</span>
          </li>
          <li className="nav-sort">
            {/* highlight class name： active */}
            <span className='nav-item'>{tabs[0]?.text}</span>
            <span className='nav-item'>{tabs[1]?.text}</span>
          </li>
        </ul>
      </div>
      ...
      <div className="reply-wrap">
        ...
        <div className="reply-list">
          {/* comment item */}
          {userList?.map(({rpid, user, content, ctime, like}) => (
            ...
            <div className="content-wrap">
              ...
              <div className="root-reply">
                ...
                <div className="reply-info">
                  ...
                  {user?.uid === currentUser?.uid ? (<span className="delete-btn" onClick={() => handleDeleteOnClick(rpid)}>
                    Delete
                  </span>): null}
                </div>
              </div>
